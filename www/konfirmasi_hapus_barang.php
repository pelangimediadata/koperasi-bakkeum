...
if (!$is_admin) {
    echo "<script>alert('Akses Ditolak! Hanya Admin yang berhak menghapus data barang.'); window.location='toko.php';</script>";
    exit();
}

$id = $_GET['id'] ?? '';
$nama = $_GET['nama'] ?? 'Barang ini';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Konfirmasi Hapus Barang - AprilNet</title>
    <style>
...
<body>
    <div class="box">
        <h3 style="color: #00f2ff;">⚠️ Konfirmasi Hapus Barang</h3>
        <p style="margin: 20px 0; color: #94a3b8;">Apakah Anda yakin ingin menghapus data barang <b><?php echo htmlspecialchars($nama); ?></b> beserta riwayat belanja/kas terkait secara permanen?</p>
        <a href="hapus_barang.php?id=<?php echo urlencode($id); ?>" class="btn btn-danger">Ya, Hapus</a>
